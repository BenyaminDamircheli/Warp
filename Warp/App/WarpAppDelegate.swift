...
private let appLogger = WarpLog.app
private let cacheLogger = WarpLog.caches

class WarpAppDelegate: NSObject, NSApplicationDelegate {
	...
	private var shouldOpenForegroundUIOnLaunch: Bool {
		!(launchedAtLogin && !warpSettings.showDockIcon)
	}
	...
	private func startLifecycleTasksIfNeeded() {
		Task { @MainActor in
			await WarpApp.appStore.send(.task).finish()
		}
	}
    ...
    private func configureLocalCaches() {
        do {
            let cache = try URL.warpApplicationSupport.appendingPathComponent("cache", isDirectory: true)
	...
	private func updateAppMode() {
		appLogger.debug("showDockIcon = \(self.warpSettings.showDockIcon)")
		if self.warpSettings.showDockIcon {
