...
- 083513c: Add comprehensive documentation to HotKeyProcessor and extract magic numbers into named constants (WarpCoreConstants)
...
- ea42b5b: Move `WarpSettings` + `RecordingAudioBehavior` into WarpCore and add fixtures/tests so we can migrate historic settings blobs safely before shipping new media-ducking options.
...
- WarpSettings now lives inside WarpCore with fixture-based migration tests, giving us a single source of truth for future settings changes.
