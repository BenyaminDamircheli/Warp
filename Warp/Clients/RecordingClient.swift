...
private let recordingLogger = WarpLog.recording
private let mediaLogger = WarpLog.media
...
struct RecordingClient {
  ...
  /// Receives cloned 16 kHz mono float PCM while recording when using the capture engine.
  var setStreamingPCMHandler: @Sendable (@escaping @Sendable (AVAudioPCMBuffer) -> Void) async -> Void = { _ in }
  var clearStreamingPCMHandler: @Sendable () async -> Void = {}
  /// Whether the most recent `stopRecording` ended a capture-engine session (vs AVAudioRecorder fallback).
  var lastRecordingUsedCaptureEngine: @Sendable () async -> Bool = { true }
}
...
extension RecordingClient: DependencyKey {
  static var liveValue: Self {
    ...
    return Self(
      ...
      setStreamingPCMHandler: { await live.setStreamingPCMHandler($0) },
      clearStreamingPCMHandler: { await live.clearStreamingPCMHandler() },
      lastRecordingUsedCaptureEngine: { await live.lastRecordingUsedCaptureEngine() }
    )
  }
}
...
actor RecordingClientLive {
  ...
  private var lastSessionUsedCaptureEngine = true
  ...
  private func handleCaptureEnvironmentChange(reason: String, forceRestart: Bool) async {
    ...
    if warpSettings.superFastModeEnabled {
  ...
  private func resolvePreferredInputDevice() -> AudioDeviceID? {
    if let selectedDeviceIDString = warpSettings.selectedMicrophoneID,
  ...
  private func currentCaptureMode() -> CaptureRecordingMode {
    warpSettings.superFastModeEnabled ? .superFast : .standard
  }
  ...
  private func makeCaptureRecordingURL() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent("warp-capture-\(UUID().uuidString).wav")
  }
  ...
  private func ensureInputDeviceUnmuted() {
    ...
    if let selectedIDString = warpSettings.selectedMicrophoneID,
  ...
  func startRecording() async {
    ...
    switch warpSettings.recordingAudioBehavior {
    case .pauseMedia:
  ...
  func stopRecording() async -> URL {
    let sessionBackendSnapshot = activeRecordingSession?.backend
    defer {
      if let sessionBackendSnapshot {
        lastSessionUsedCaptureEngine = (sessionBackendSnapshot == .captureEngine)
      }
    }
  ...
  private func duplicateCurrentRecording() throws -> URL {
    ...
    let exportURL = recordingURL
      ...
      .appendingPathComponent("warp-recording-\(UUID().uuidString).wav")
  ...
  func warmUpRecorder() async {
    ...
    if warpSettings.superFastModeEnabled {
      ...
      do {
        ...
        recordingLogger.error("Failed to arm capture engine for Warp Speed: \(error.localizedDescription)")
      }
      return
    }
  ...
  func setStreamingPCMHandler(_ handler: @escaping @Sendable (AVAudioPCMBuffer) -> Void) async {
    captureController.setStreamingPCMHandler { buffer in
      handler(buffer)
    }
  }

  func clearStreamingPCMHandler() async {
    captureController.setStreamingPCMHandler(nil)
  }

  func lastRecordingUsedCaptureEngine() async -> Bool {
    lastSessionUsedCaptureEngine
  }
