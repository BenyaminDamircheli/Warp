...
actor SoundEffectsClientLive {
  private let logger = WarpLog.sound
  private let baselineVolume = WarpSettings.baseSoundEffectsVolume
  ...
  func play(_ soundEffect: SoundEffect) {
	guard warpSettings.soundEffectsEnabled else { return }
	...
	let clampedVolume = min(max(warpSettings.soundEffectsVolume, 0), baselineVolume)
