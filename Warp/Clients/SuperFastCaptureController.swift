...
final class SuperFastCaptureController {
  ...
  private let logger = WarpLog.recording
  private let processingQueue = DispatchQueue(label: "com.benyamindamircheli.warp.SuperFastCapture")
  ...
  /// Optional tap for streaming ASR (16 kHz mono float buffers). Invoked on `processingQueue`.
  private var streamingPCMHandler: ((AVAudioPCMBuffer) -> Void)?
  ...
  func setStreamingPCMHandler(_ handler: ((AVAudioPCMBuffer) -> Void)?) {
    processingQueue.async { [weak self] in
      self?.streamingPCMHandler = handler
    }
  }
  ...
  private func process(_ buffer: AVAudioPCMBuffer) {
    ...
    if activeRecording != nil, let handler = streamingPCMHandler, let pcmCopy = clone(converted) {
      handler(pcmCopy)
    }
