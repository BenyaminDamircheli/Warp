...
private let transcriptionLogger = WarpLog.transcription
private let modelsLogger = WarpLog.models
private let parakeetLogger = WarpLog.parakeet
...
struct TranscriptionClient {
  ...
  /// Loads Parakeet EOU streaming weights and clears decoder state for a new utterance.
  var beginStreamingParakeetSession: @Sendable (String, @escaping (Progress) -> Void) async throws -> Void = { _, _ in }

  /// Feeds one converted PCM buffer (16 kHz mono float) into the streaming EOU pipeline.
  var feedStreamingParakeetBuffer: @Sendable (AVAudioPCMBuffer) async throws -> Void = { _ in }

  /// Finalizes streaming ASR and returns the transcript (call after `stopRecording`).
  var finishStreamingParakeetTranscription: @Sendable () async throws -> String = { "" }

  /// Clears streaming decoder state (e.g. cancel / discard).
  var resetStreamingParakeet: @Sendable () async -> Void = {}
}
...
extension TranscriptionClient: DependencyKey {
  static var liveValue: Self {
    ...
    return Self(
      ...
      beginStreamingParakeetSession: { try await live.beginStreamingParakeetSession(model: $0, progressCallback: $1) },
      feedStreamingParakeetBuffer: { try await live.feedStreamingParakeetBuffer($0) },
      finishStreamingParakeetTranscription: { try await live.finishStreamingParakeetTranscription() },
      resetStreamingParakeet: { await live.resetStreamingParakeet() }
    )
  }
}
...
actor TranscriptionClientLive {
  ...
  private lazy var modelsBaseFolder: URL = {
    do {
      return try URL.warpModelsDirectory
    } catch {
  ...
  func isModelDownloaded(_ modelName: String) async -> Bool {
    if isParakeetEOU(modelName) {
      let available = await streamingParakeet.isModelAvailable(modelName)
      parakeetLogger.debug("Parakeet EOU available? \(available)")
      return available
    }
    if isParakeetTDT(modelName) {
  ...
  func transcribe(
    ...
    let startAll = Date()
    if isParakeetEOU(model) {
      transcriptionLogger.notice("Transcribing with Parakeet EOU model=\(model) file=\(url.lastPathComponent)")
      let startLoad = Date()
      try await downloadAndLoadModel(variant: model) { p in
        progressCallback(p)
      }
      transcriptionLogger.info("Parakeet EOU ensureLoaded took \(String(format: "%.2f", Date().timeIntervalSince(startLoad)))s")
      let preparedClip = try ParakeetClipPreparer.ensureMinimumDuration(url: url, logger: parakeetLogger)
      defer { preparedClip.cleanup() }
      let startTx = Date()
      let text = try await streamingParakeet.transcribeFile(preparedClip.url)
      transcriptionLogger.info("Parakeet EOU transcription took \(String(format: "%.2f", Date().timeIntervalSince(startTx)))s")
      transcriptionLogger.info("Parakeet EOU request total elapsed \(String(format: "%.2f", Date().timeIntervalSince(startAll)))s")
      return text
    }
    if isParakeetTDT(model) {
  ...
  func beginStreamingParakeetSession(model: String, progressCallback: @escaping (Progress) -> Void) async throws {
    try await streamingParakeet.beginSession(modelName: model, progress: progressCallback)
  }

  func feedStreamingParakeetBuffer(_ buffer: AVAudioPCMBuffer) async throws {
    try await streamingParakeet.process(buffer: buffer)
  }

  func finishStreamingParakeetTranscription() async throws -> String {
    try await streamingParakeet.finish()
  }

  func resetStreamingParakeet() async {
    await streamingParakeet.reset()
  }

  private func isParakeetTDT(_ name: String) -> Bool {
    ParakeetModel(rawValue: name)?.isTDT == true
  }

  private func isParakeetEOU(_ name: String) -> Bool {
    ParakeetModel(rawValue: name)?.isStreamingEOU == true
  }
