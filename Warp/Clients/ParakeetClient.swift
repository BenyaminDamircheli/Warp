...
actor ParakeetClient {
  ...
  private let logger = WarpLog.parakeet
  private let vendorDirs = [
    // Our app-specific cache path convention (under XDG or Application Support cache)
    "fluidaudio/Models",
    "FluidAudio/Models"
  ]
  ...
  private func candidateRoots() -> [URL] {
    ...
    let appCache = try? URL.warpApplicationSupport.appendingPathComponent("cache", isDirectory: true)
...
private extension ParakeetModel {
  var asrVersion: AsrModelVersion {
    ...
    case .eouStreaming160:
      fatalError("TDT AsrManager does not load EOU; use StreamingParakeetClient")
    }
  }
}
...
actor ParakeetClient {
  ...
  func ensureLoaded(modelName: String, progress: @escaping (Progress) -> Void) async throws {
    throw NSError(
      domain: "Parakeet",
      code: -2,
      userInfo: [NSLocalizedDescriptionKey: "Parakeet support not linked. Add Swift Package: https://github.com/FluidInference/FluidAudio.git and link FluidAudio to the Warp target."]
    )
  }
