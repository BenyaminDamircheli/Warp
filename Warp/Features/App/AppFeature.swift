...
struct AppFeature {
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case .transcription(.modelMissing):
        WarpLog.app.notice("Model missing - activating app and switching to settings")
        state.activeTab = .settings
        state.settings.shouldFlashModelSection = true
        return .run { send in
          await MainActor.run {
            WarpLog.app.notice("Activating app for model missing")
  ...
  private func startPasteLastTranscriptMonitoring() -> Effect<Action> {
    .run { send in
      ...
      let token = keyEventMonitor.handleKeyEvent { keyEvent in
        ...
        guard let pasteHotkey = warpSettings.pasteLastTranscriptHotkey,
  ...
  private func ensureSelectedModelReadiness() -> Effect<Action> {
    .run { send in
      @Shared(.warpSettings) var warpSettings: WarpSettings
      @Shared(.modelBootstrapState) var modelBootstrapState: ModelBootstrapState
      let selectedModel = warpSettings.selectedModel
