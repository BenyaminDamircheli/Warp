...
struct SettingsFeature {
  ...
  struct State {
    ...
    /// Whether a non-empty Inception API key exists in Keychain (never stores the secret in state).
    var hasInceptionAPIKey: Bool = false
  ...
  enum Action: BindableAction {
    ...
    case loadInceptionAPIKeyPresence
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      switch action {
      case .binding:
        let didNormalizeDoubleTapOnly = !state.warpSettings.doubleTapLockEnabled && state.warpSettings.useDoubleTapOnly
      ...
      case .clearPasteLastTranscriptHotkey:
        state.$warpSettings.withLock { $0.pasteLastTranscriptHotkey = nil }
        return .none
      ...
      case let .toggleOpenOnLogin(enabled):
        state.$warpSettings.withLock { $0.openOnLogin = enabled }
        return .run { _ in
          if enabled {
      ...
      case let .toggleShowDockIcon(enabled):
        state.$warpSettings.withLock { $0.showDockIcon = enabled }
        return .run { _ in
          await MainActor.run {
      ...
      case let .togglePreventSystemSleep(enabled):
        state.$warpSettings.withLock { $0.preventSystemSleep = enabled }
        return .none

      case let .setRecordingAudioBehavior(behavior):
        state.$warpSettings.withLock { $0.recordingAudioBehavior = behavior }
        return .none

      case let .toggleSuperFastMode(enabled):
        state.$warpSettings.withLock { $0.superFastModeEnabled = enabled }
        return .run { _ in
      ...
      case let .toggleSaveTranscriptionHistory(enabled):
        state.$warpSettings.withLock { $0.saveTranscriptionHistory = enabled }
      ...
      case let .setModifierSide(kind, side):
        guard state.warpSettings.hotkey.key == nil else { return .none }
        state.$warpSettings.withLock {
      ...
      case .loadInceptionAPIKeyPresence:
        return .run { send in
          do {
            ...
            await send(.inceptionAPIKeyPresenceLoaded(hasKey))
          } catch {
            settingsLogger.error("Failed to read Inception API key presence: \(error.localizedDescription)")
            await send(.inceptionAPIKeyPresenceLoaded(false))
          }
        }

      case let .inceptionAPIKeyPresenceLoaded(hasKey):
        state.hasInceptionAPIKey = hasKey
        return .none

      case let .saveInceptionAPIKey(key):
        let trimmed = key.trimmingCharacters(in: .whitespacesAndNewlines)
        return .run { send in
          if trimmed.isEmpty {
            try await inceptionAPIKey.delete()
            await send(.inceptionAPIKeyPresenceLoaded(false))
          } else {
            try await inceptionAPIKey.save(trimmed)
            await send(.inceptionAPIKeyPresenceLoaded(true))
          }
        }
