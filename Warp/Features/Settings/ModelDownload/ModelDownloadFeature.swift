...
public struct ModelDownloadFeature {
	...
	public struct State: Equatable {
		...
		var selectedModel: String { warpSettings.selectedModel }
	...
	private func reduce(state: inout State, action: Action) -> Effect<Action> {
		...
		case let .selectModel(model):
			...
			state.$warpSettings.withLock { $0.selectedModel = resolved }
		...
		case let .modelsLoaded(recommended, available):
			...
			if state.warpSettings.selectedModel.contains("*") || state.warpSettings.selectedModel.contains("?") {
				if let resolved = resolvePattern(state.warpSettings.selectedModel, from: available) {
					state.$warpSettings.withLock { $0.selectedModel = resolved }
				}
			}
			...
			updateBootstrapState(&state)
			if !state.anyModelDownloaded && !state.warpSettings.hasCompletedModelBootstrap {
				let preferred = state.recommendedModel.isEmpty ? state.warpSettings.selectedModel : state.recommendedModel
				if !preferred.isEmpty {
					state.$warpSettings.withLock { $0.selectedModel = preferred }
					updateBootstrapState(&state)
				}
			}
			return .none
		...
		case .downloadSelectedModel:
			guard !state.warpSettings.selectedModel.isEmpty else { return .none }
			state.downloadError = nil
			state.isDownloading = true
			let selected = state.warpSettings.selectedModel
		...
		case let .downloadProgress(progress):
			...
			if state.downloadingModelName == state.warpSettings.selectedModel {
	...
	private func openModelLocationEffect() -> Effect<Action> {
		.run { _ in
			let base = try URL.warpModelsDirectory
...
extension ModelDownloadFeature.State {
	...
	private var prefersEnglishParakeet: Bool {
		guard let language = warpSettings.outputLanguage?.lowercased(), !language.isEmpty else {
			return false
		}
