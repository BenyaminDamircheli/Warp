...
struct MicrophoneSelectionSectionView: View {
	...
	private var selectedDeviceTag: Binding<String> {
		Binding(
			get: { store.warpSettings.selectedMicrophoneID ?? Self.systemDefaultTag },
			set: { newValue in
				store.warpSettings.selectedMicrophoneID = newValue == Self.systemDefaultTag ? nil : newValue
			}
		)
	}

	private var missingSelectedDeviceID: String? {
		guard let selectedID = store.warpSettings.selectedMicrophoneID,
	...
	private var pickerIdentity: String {
		[
			...
			store.warpSettings.selectedMicrophoneID ?? Self.systemDefaultTag,
