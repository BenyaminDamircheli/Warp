...
struct HotKeySectionView: View {
    ...
    var body: some View {
        Section("Hot Key") {
            let hotKey = store.warpSettings.hotkey
            ...
            Label {
                Toggle("Enable double-tap lock", isOn: $store.warpSettings.doubleTapLockEnabled)
            } icon: {
            ...
            if hotKey.key != nil {
                Label {
                    Toggle("Use double-tap only", isOn: $store.warpSettings.useDoubleTapOnly)
                        .disabled(!store.warpSettings.doubleTapLockEnabled)
                } icon: {
            ...
            if store.warpSettings.hotkey.key == nil {
                Label {
                    Slider(value: $store.warpSettings.minimumKeyTime, in: 0.0 ... 2.0, step: 0.1) {
                        Text("Ignore below \(store.warpSettings.minimumKeyTime, specifier: "%.1f")s")
                    }
                } icon: {
