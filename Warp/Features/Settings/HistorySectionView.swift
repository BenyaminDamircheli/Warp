		...
		Section {
			Label {
				Toggle("Save Transcription History", isOn: Binding(
					get: { store.warpSettings.saveTranscriptionHistory },
			...
			if store.warpSettings.saveTranscriptionHistory {
				Label {
					HStack {
						Text("Maximum History Entries")
						Spacer()
						Picker("", selection: Binding(
							get: { store.warpSettings.maxHistoryEntries ?? 0 },
							set: { newValue in
								store.warpSettings.maxHistoryEntries = newValue == 0 ? nil : newValue
							}
						)) {
				...
				if store.warpSettings.maxHistoryEntries != nil {
...
private struct PasteLastTranscriptHotkeyRow: View {
	...
	var body: some View {
		let pasteHotkey = store.warpSettings.pasteLastTranscriptHotkey
