...
struct WordRemappingsView: View {
	...
	private var removalsSection: some View {
		GroupBox {
			VStack(alignment: .leading, spacing: 10) {
				Toggle("Enable Word Removals", isOn: $store.warpSettings.wordRemovalsEnabled)
	...
	private var previewText: String {
		...
		if store.warpSettings.wordRemovalsEnabled {
			output = WordRemovalApplier.apply(output, removals: store.warpSettings.wordRemovals)
		}
		output = WordRemappingApplier.apply(output, remappings: store.warpSettings.wordRemappings)
		return output
	}
}
