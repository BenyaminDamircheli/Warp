...
2. Keep padding transparent to users: no audible artifacts, no extra latency, and logs that explain when/why padding ran (`WarpLog.parakeet`).
...
2. Insert the inspector inside `TranscriptionClient` right before the Parakeet branch so we can log current clip lengths (`WarpLog.parakeet.debug("clip=0.18s")`).
...
2. Unit test this helper in `WarpCoreTests` with fixtures (<50 ms clip, >threshold clip) to guarantee correct math and metadata.
3. Make the minimum configurable (env var or `WarpSettings`) so we can tweak without hard-coding; default to whatever FluidAudio recommends.
