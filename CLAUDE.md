# Warp – Dev Notes for Agents

This file provides guidance for coding agents working in this repo.

## Project Overview

Warp is a macOS menu bar application for on‑device voice‑to‑text. It supports Whisper (Core ML via WhisperKit) and Parakeet TDT v3 (Core ML via FluidAudio). Users activate transcription with hotkeys; text can be auto‑pasted into the active app.
...
# Run tests (must be run from WarpCore directory for unit tests)
cd WarpCore && swift test

# Or run all tests via Xcode
xcodebuild test -scheme Warp
...
- **Sparkle**: Auto-updates (`SUFeedURL` in `Warp/Info.plist` — point to your appcast)
- **Swift Composable Architecture**: State management
- **Inject** Hot Reloading for SwiftUI

## Important Implementation Details

1. **Hotkey Recording Modes**: The app supports both press-and-hold and double-tap recording modes, implemented in `HotKeyProcessor.swift`. Detailed behavior includes:
...
2. **Model Management**: Models are managed by `ModelDownloadFeature`. Curated defaults live in `Warp/Resources/Data/models.json`. The Settings UI shows a compact opinionated list (Parakeet + three Whisper sizes). No dropdowns.
...
5. **Permissions**: Requires audio input and automation entitlements (see `Warp/Warp.entitlements`)

6. **Logging**: All diagnostics should use the unified logging helper `WarpLog` (`WarpCore/Sources/WarpCore/Logging.swift`). Pick an existing category (e.g., `.transcription`, `.recording`, `.settings`) or add a new case so Console predicates stay consistent. Avoid `print` and prefer privacy annotations (`, privacy: .private`) for anything potentially sensitive like transcript text or file paths.
...
- WhisperKit models
  - `~/Library/Application Support/com.benyamindamircheli.warp/models/argmaxinc/whisperkit-coreml/<model>`
- Parakeet (FluidAudio)
  - We set `XDG_CACHE_HOME` on launch so Parakeet caches under the app container:
  - `~/Library/Containers/com.benyamindamircheli.warp/Data/Library/Application Support/FluidAudio/Models/parakeet-tdt-0.6b-v3-coreml`
...
- FluidAudio: `https://github.com/FluidInference/FluidAudio.git` (link `FluidAudio` to Warp target)
...
```
XDG_CACHE_HOME = ~/Library/Containers/com.benyamindamircheli.warp/Data/Library/Application Support/com.benyamindamircheli.warp/cache
```
...
- Parakeet not detected: ensure it resides under the container path above; downloading from Warp places it correctly.
...
2. **Use non-interactive changeset creation:** AI agents should use the non-interactive script:
   ...
   ```
3. **Only create changesets, don't process them:** Agents should only create changeset fragments. The release tool is responsible for running `changeset version` to collect changesets into `CHANGELOG.md` and syncing to `Warp/Resources/changelog.md`.
...
3. Syncs changelog to `Warp/Resources/changelog.md`
...
12. Uploads to S3 (versioned DMG + `warp-latest.dmg` + appcast.xml — configure bucket/paths in release tooling)
...
- `Warp-{version}.dmg` - Signed, notarized DMG
- `Warp-{version}.zip` - For Homebrew cask
- `warp-latest.dmg` - Always points to latest (if configured)
